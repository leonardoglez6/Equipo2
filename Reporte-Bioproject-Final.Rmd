---
title: "Reporte BioProject Equipo 2"
author: "Leonardo González López, Yuliana Denisse Sosa Gómez"
date: "`r Sys.Date()`"
output: html_document
editor_options: 
  markdown: 
    wrap: 72
---

# Descripción del equipo

-   **Equipo:** 2

-   **Integrantes:**

    -   Leonardo González López (lgonzalez) -
        [minestev17\@gmail.com](mailto:minestev17@gmail.com){.email},
    -   Yuliana Denisse Sosa Gómez (ysosa) -
        [yuliana.sogo2006\@gmail.com](mailto:yuliana.sogo2006@gmail.com){.email}

# Descripción de los datos

+------------------+---------------------------------------------------+
| Descripción      | Información                                       |
+==================+===================================================+
| BioProject       | PRJNA1198873                                      |
+------------------+---------------------------------------------------+
| Especie          | *Homo sapiens*                                    |
+------------------+---------------------------------------------------+
| Tipo de          | *paired-end*                                      |
| bibliotecas      |                                                   |
+------------------+---------------------------------------------------+
| Método de        | Purificación de transcritos poliadenilados        |
| selección        | (poli-A)                                          |
+------------------+---------------------------------------------------+
| Número de        | 6                                                 |
| transcriptomas   |                                                   |
+------------------+---------------------------------------------------+
| Número de        | 3 replicas biológicas por condición (control y    |
| réplicas         | knockdown)                                        |
| biológicas       |                                                   |
+------------------+---------------------------------------------------+
| Secuenciador     | Illumina NovaSeq 6000                             |
| empleado         |                                                   |
+------------------+---------------------------------------------------+
| Distribución de  | -   **Control**: 3 réplicas biológicas WT         |
| las muestras     |     (SRR31732089, SRR31732090, SRR31732091)       |
|                  |                                                   |
|                  | -   **Sobreexpresión**: 3 réplicas biológicas con |
|                  |     knockdown de *STEAP3* (SRR31732086,           |
|                  |     SRR31732087, SRR31732088)\n                   |
+------------------+---------------------------------------------------+
| Profundidad de   | 19 - 24 M seq                                     |
| secuenciación de |                                                   |
| cada             |                                                   |
| transcriptoma    |                                                   |
+------------------+---------------------------------------------------+
| Tamaño de las    | 148 bp                                            |
| lecturas         |                                                   |
+------------------+---------------------------------------------------+
| Artículo         | Zhao Z, Yu P, Wang Y, Li H et al. Silencing of    |
| científico       | STEAP3 suppresses cervical cancer cell            |
|                  | proliferation and migration via JAK/STAT3         |
|                  | signaling pathway. Cancer Metab 2024 Dec          |
|                  | 30;12(1):40. PM ID: [3973 675                     |
|                  | 1](htt%20%20%2%2                                  |
|                  | 00%2%2%20%2000%20%20%20%20%20p%%20%20%%202020%%20 |
|                  | 2%200s:%20%20%20%20%2%200//%20%%202%200ww%20w%%20 |
|                  | %%202020%%2%%20200%2020%%2020.%2%%20%20%20200n%%2 |
|                  | 0%202%200c%20%2%200bi%2%200%20%2%20%200.n%2%200%% |
|                  | 20%2020%%2%20020%20%20l%20%2%200m.n%%%2020%202%%2 |
|                  | 020%200i%%2%2002%%20200%20%%%2%20%200202%200h%2%% |
|                  | 20200.%2%20%20%200g%%20%20%%202%20%200%2020o%%20% |
|                  | 202%20%200%20%20%20%20v%20%20/p%20%20%%2020ub%%20 |
|                  | 20%%202%20%200m%20%20ed/%203%2%2%2000%%20%%202020 |
|                  | 9%20%20%%20%20%2%200%20%2020%%%20%20%2020207%2%2% |
|                  | 200%%20%20%20%2%2002003%20%%202%200%%%20%20%2%200 |
|                  | 202%20%2%2000%2%%20%20200%20%206%%20207%20%%%2020 |
|                  | %%2020%2%200%202%%20%20200%205%20%%20%20%202%200% |
|                  | 20%201%20%22Link%20t%20o%20%20PubMed%20record%22) |
+------------------+---------------------------------------------------+

: Tabla 1: Detalles del Bioproyecto y características de las muestras

## Resumen del BioProject

Basado en el artículo "Silencing of *STEAP3* suppresses cervical cancer
cell proliferation and migration via *JAK/STAT3* signaling pathway” se
comparó el perfil transcripcional, a partir de secuencias en archivos
FASTQ de 6 muestras de la línea celular HeLa divididas en 2 grupos, uno
control y otro al que se deletó el gen *STEAP3*. Se realizó un
alineamiento y conteo con STAR debido a la facilidad de trabajar con
este programa ya que no se necesita crear un archivo .txi para importar
los datos a R; posteriormente se realizó el análisis de expresión
diferencial con DESeq2 y el enriquecimiento funcional con gProfiler2 y
clusterprofiler para obtener un enriquecimiento de GO Biological Process
y de KEGG pathways. Se encontraron 3 genes subregulados y 12 genes
sobreexpresados, destacando los genes *SERPINB3/4* asociados a la
sensibilidad al tratamiento de cáncer cervical, además el gen *STEAP3*
se encuentra subregulado como era esperado. La vía de la ferrotopsis se
encuentra enriquecida debido a la subregulación de *STEAP3*, la cuál
está relacionada con la muerte celular, el cuál es un mecanismo
comúnmente relacionado con el cáncer.

Se utilizaron todas las muestras del dataset, correspondiendo a 3
réplicas biológicas de células de la línea celular HeLA con knockdown en
el gen *STEAP3*. La información se encuentra en la **Tabla 2**.

```{r echo=FALSE, message=FALSE, warning=FALSE}
library(DT)

# ejemplo de data frame
df <- data.frame(
  muestras = c("GSM8682795", "GSM8682794", "GSM8682793", "GSM8682792", "GSM8682791", "GSM8682790"),
  condicion = c("STEAP3 knockdown", "STEAP3 knockdown", "STEAP3 knockdown", "WT", "WT", "WT"),
  SampleID = c("SAMN45855841", "SAMN45855842", "SAMN45855843",
                "SAMN45855844", "SAMN45855845", "SAMN45855846"),
  replica_biologica = c("KD_3", "KD_2", "KD_1",
                        "NC_3", "NC_2", "NC_1")
)

# tabla interactiva
datatable(
  df,
  caption = "Tabla 2. Distribución de las muestras y réplicas biológicas",
  options = list(
    pageLength = 6,   # Show 6 entries
    lengthMenu = c(6, 10, 20),
    searching = TRUE,
    paging = TRUE
  )
)
```

# Diagrama de flujo de trabajo

![](figures/Diagrama_de_flujo.png)

# Estructura del trabajo

La **estructura** de las carpetas dentro del GitHub se puede observar a
continuación, la cual contiene carpetas compartidas con la estructura
dentro del cluster:

```         
├── Equipo2.Rproj                                         # Proyecto en R                         
├── README.md                                             # Archivo README del proyecto.
├── Reporte-Bioproject-Final.Rmd                          # Archivo con reporte documentado
├── Reporte-Bioproject-Final.html                         # Archivo renderizado del reporte (este archivo)
├── Infografia-Cientifica-Proyecto                        # Infografía Científica para el público general
├── metadata.csv                                          # Metadata de proyecto utilizada en scripts
├── figures                                               # Carpeta con imagenes usadas en reporte
│   ├── Diagrama_de_flujo.png                               # Workflow del proyecto
│   ├── Dotplot-articulo-original.jpeg                      # Dotplot artículo de referencia
│   ├── Heatmap_KD_vs_WT.png                                # Heatmap 20 genes más expresados diferencialmente
│   ├── KEGG_droplot.png                                    # Droplot con pathways KEGG más enriquecidos
│   ├── ManhattanGO_KD_vs_WT.png                            # Manhattan plot de Geno Ontology (GO)
│   ├── PCA_no_batch_rlog.png                               # PCA sin batch effect
│   ├── PCA_rlog.png                                        # PCA con batch effect
│   ├── VolcanoPlot_KD_vs_WT.png                            # Volcano Plot genes sobreexpresados y subexpresados
│   ├── VolcanoPlot-articulo-original.jpeg                  # Volcano Plot artículo de referencia
│   ├── barplotDOWN_GO_KD_vs_WT.png                         # Barplot 40 términos más subexpresados 
│   ├── barplotUP_GO_KD_vs_WT.png                           # Barplot 40 términos más sobreexpresados
│   ├── fastqc_adapters_processed.png                       # Imagenes de control de calidad
│   ├── fastqc_adapters_raw.png                               # raw: antes de trimming
│   ├── fastqc_per_base_n_content_plot_processed.png          # processed: después de trimming
│   ├── fastqc_per_base_n_content_plot_raw.png
│   ├── fastqc_per_base_sequence_quality_plot_processed.png
│   ├── fastqc_per_base_sequence_quality_plot_raw.png
│   ├── fastqc_per_sequence_gc_content_plot_processed.png
│   ├── fastqc_per_sequence_gc_content_plot_raw.png
│   ├── fastqc_per_sequence_quality_scores_plot_processed.png
│   ├── fastqc_per_sequence_quality_scores_plot_raw.png
│   ├── fastqc_sequence_duplication_levels_plot_processed.png
│   ├── fastqc_sequence_duplication_levels_plot_raw.png
│   ├── fastqc_sequence_length_distribution_plot_processed.png
│   └── fastqc_sequence_length_distribution_plot_raw.png
├── out_logs                                              # Outlogs de jobs de slurm corridos en Ken
│   ├── DEG_STAR_KD1685.err
│   ├── DEG_STAR_KD1685.out
│   ├── GO_terms_STAR1687.err
│   ├── GO_terms_STAR1687.out
│   ├── Load_data_STAR_1684.err
│   ├── Load_data_STAR_1684.out
│   ├── SRR31732086.ReadsPerGene.out.tab
│   ├── SRR31732087.ReadsPerGene.out.tab
│   ├── SRR31732088.ReadsPerGene.out.tab
│   ├── SRR31732089.ReadsPerGene.out.tab
│   ├── SRR31732090.ReadsPerGene.out.tab
│   ├── SRR31732091.ReadsPerGene.out.tab
│   ├── Visualization_data_STAR1686.err
│   ├── Visualization_data_STAR1686.out
│   ├── kallisto_1400.err
│   ├── kallisto_1400.out
│   ├── kallisto_1477.err
│   ├── kallisto_1477.out
│   ├── multiqc_1285.err
│   ├── multiqc_1285.out
│   ├── multiqc_1425.err
│   ├── multiqc_1425.out
│   ├── multiqc_1479.err
│   ├── multiqc_1479.out
│   ├── r_count_txi_1527.err
│   ├── r_count_txi_1527.out
│   ├── r_count_txi_1528.err
│   ├── r_count_txi_1528.out
│   ├── rnaseq_1481.err
│   ├── rnaseq_1481.out
│   ├── rnaseq_1482.err
│   ├── rnaseq_1482.out
│   ├── rnaseq_1483.err
│   ├── rnaseq_1483.out
│   ├── rnaseq_1489.err
│   ├── rnaseq_1489.out
│   ├── rnaseq_1490.err
│   ├── rnaseq_1490.out
│   ├── star_align_1480.err
│   ├── star_align_1480.out
│   ├── star_index_1404.err
│   ├── star_index_1404.out
│   ├── star_index_1440.err
│   ├── star_index_1440.out
│   ├── trimm_1286.err
│   ├── trimm_1286.out
│   ├── trimm_1429.err
│   ├── trimm_1429.out
│   ├── trimm_1431.err
│   ├── trimm_1431.out
│   ├── trimm_1433.err
│   ├── trimm_1433.out
│   ├── trimm_1434.err
│   ├── trimm_1434.out
│   ├── trimm_1435.err
│   ├── trimm_1435.out
│   ├── trimm_1437.err
│   └── trimm_1437.out
├── quality                                               # Reporte de calidad Multiqc
│   ├── multiqc_report_raw.html                             # Reporte datos antes de trimming
│   └── multiqc_report_processed.html                       # Reporte datos después de trimming
├── results                                               # Resultados de scripts
│   ├── DE_KD_vs_WT.csv                                     # Matriz con DEGs, LFC y p-values
│   ├── DOWN_GOKD_vs_WT.csv                                 # Matriz 40 términos, genes subexpresados
│   ├── KEGG_enrichment_KD_vs_WT.csv                        # Matriz vías KEGG enriquecidas
│   ├── STEAP3_KD_vs_WT.RData                               # Matriz metadata y objeto dds (DESeq2)
│   ├── UP_GO_KD_vs_WT.csv                                  # Matriz 40 términos, genes sobreexpresados
│   ├── counts                                            # Cuentas crudas de 6 transcriptomas
│   │   ├── raw_counts.RData                                # Metadata y cuentas
│   │   └── raw_counts.csv                                  # Solo cuentas
│   └── figures                                           # Figuras generadas con scripts
│       ├── Heatmap_KD_vs_WT.png
│       ├── KEGG_droplot.png
│       ├── ManhattanGO_KD_vs_WT.png
│       ├── PCA_no_batch_rlog.png
│       ├── PCA_rlog.png
│       ├── VolcanoPlot_KD_vs_WT.png
│       ├── barplotDOWN_GO_KD_vs_WT.png
│       └── barplotUP_GO_KD_vs_WT.png
└── scripts                                               # Scripts documentados del proyecto
    ├── 01_DownloadRawData.sh                               # Descarga archivos FASTQ
    ├── 02_Quality_raw.sh                                   # Reportes FastQC datos crudos
    ├── 03_Trimmed.sh                                       # Trimming datos
    ├── 04_Quality_trimmed.sh                               # Reportes FastQC datos procesados
    ├── 05_STAR_index.sh                                    # Indexado de genoma referencia  
    ├── 06_STAR_align.sh                                    # Alineamiento y cuentas con STAR
    ├── 07_Load_data_STAR.R                                 # Matriz de cuentas conjunta en R
    ├── S07_Load_data_STAR.sh                               # Job slurm Script 7
    ├── 08_DEG_STAR_KD.R                                    # Corrección batch effect y análisis DEG
    ├── S08_DEG_STAR_KD.sh                                  # Job slurm Script 8
    ├── 09_Visualization_data_STAR.R                        # Genera VolcanoPlot y Heatmap
    ├── S09_Visualization_data_STAR.sh                      # Job slurm Script 9
    ├── 10_GO_terms_STAR.R                                  # Enriquecimiento de términos GO, KEGG
    └── S10_GO_terms_STAR.sh                                # Job slurm Script 10
```

## Outlogs

Los **outlogs** de los scripts se pueden observar en:
[Outlogs](https://github.com/leonardoglez6/Equipo2/tree/main/out_logs).

Y, dentro del cluster, en la ruta
`/mnt/data/bioinfo-estadistica-2/RNAseq_2026/equipos/Equipo2/scripts/out_logs`

## Paquetes y versiones

Los módulos utilizados dentro del cluster fueron los siguientes:

```         
module load anaconda3/2025.06
⎣ source activate multiqc-1.5
module load fastqc/0.11.3
module load star/2.7.9a 
module load trimmomatic/0.33
module load r/4.4.1
```

Por otro lado, los paquetes utilizados para los análisis con R fueron:

```         
library(clusterProfiler) #versión 4.14.6
library(DESeq2) #versión 1.46.0
library(DOSE) #versión 4.0.1
library(dplyr) #versión 1.1.4
library(enrichplot) #versión 1.26.6
library(ggplot2) #versión 4.0.1
library(gprofiler2) #versión 0.2.4
library(org.Hs.eg.db) #versión 3.20.0
library(pheatmap) #versión 1.0.13
library(tidyverse) #versión 2.0.0
```

# Resultados

## 1. Obtener datos crudos

Buscamos nuestro BioProject dentro de ENA (European Nucleotide Archive),
para poder seleccionar las muestras. Posteriormente, utilizamos el
comando `wget` para las 12 muestras (6 muestras con su respectiva
lectura forward y reverse).

El script correspondiente se puede encontrar dentro del GitHub, en la
ruta
[scripts/01_DownloadRawData.sh](https://github.com/leonardoglez6/Equipo2/blob/main/scripts/01_DownloadRawData.sh)

Los datos correspondientes, se almacenaron en el cluster, en la ruta
`/mnt/data/bioinfo-estadistica-2/RNAseq_2026/equipos/Equipo2/data/raw`

## 2. Análisis de calidad de las secuencias de los datos crudos

Se realizó el análisis de calidad de las 6 muestras seleccionadas con el
programa **FastQC**, primero de forma individual y después generando un
MultiQC.

El script correspondiente se puede encontrar dentro del GitHub, en la
ruta
[scripts/02_Quality_raw.sh](https://github.com/leonardoglez6/Equipo2/blob/main/scripts/02_Quality_raw.sh)

Los resultados del análisis de calidad, se almacenaron en el cluster, en
la ruta
`/mnt/data/bioinfo-estadistica-2/RNAseq_2026/equipos/Equipo2/quality1`

Se obtuvo un contenido de GC aproximado de 50% formando una distribución
normal, característica de un resultado esperado de buena calidad, y una
cobertura que varía del rango de 19M - 24.7M.

![Contenido de GC de los datos
crudos](figures/fastqc_per_sequence_gc_content_plot_raw.png)

Las muestras tienen una calidad óptima dentro de las posiciones de las
lecturas, con valores dentro de la zona verde y un puntaje Phred
superior a 35. De igual forma, las muestras tienen un promedio de
calidad con un pico máximo en Phred 36.

![Calidad por base de los datos
crudos](figures/fastqc_per_base_sequence_quality_plot_raw.png)

![Calidad por secuencia de los datos
crudos](figures/fastqc_per_sequence_quality_scores_plot_raw.png)

El porcentaje de contenido de bases indeterminadas (N) en los reads
superó la prueba, así como la prueba de secuencias sobrerepresentadas y
no se encontraron muestras con contaminación de adaptadores.

![Contenido de bases indeterminadas de los datos
crudos](figures/fastqc_per_base_n_content_plot_raw.png)

![Pruebas secuencias sobrerrepresentadas y contenido de adaptadores de
los datos crudos](figures/fastqc_adapters_raw.png)

Sin embargo, se identificaron secuencias duplicadas, que posiblemente
puedan estar relacionadas con la presencia de dímeros de adaptadores.

![Secuencias duplicadas de los datos
crudos](figures/fastqc_sequence_duplication_levels_plot_raw.png)

### Conclusión de la calidad de los datos crudos

Al terminar de analizar los datos, concluimos que son adecuados para el
desarrollo del proyecto. Por lo general, las secuencias tienen buena
calidad con valores Phred mayores a 30 y tienen un porcentaje de GC
correcto. Además de que los posibles errores por secuencias duplicadas
se podrían mejorar al remover los adaptadores.

Podemos concluir que podemos continuar utilizando esta base de datos
para análisis futuros, pero debemos realizar la limpieza correspondiente
para mejorar la calidad de los mismos.

## 3. Trimming de datos crudos

Se realizó el trimming de los datos para quitar secuencias de mala
calidad y remover la posible presencia de adaptadores dentro de los
datos.

De manera previa, se descargaron las secuencias de los adaptadores a
eliminar *TruSeq3 Paired End* debido a las características de nuestros
datos. Posteriormente, se utilizó el programa **Trimmomatic** con los
siguientes parámetros:

-   `LEADING:3` Elimina bases del inicio del read si su calidad es menor
    a 3

-   `TRAILING:3` Elimina bases del final del read si su calidad es menor
    a 3

-   `SLIDINGWINDOW:4:20` Recorre read con ventana de 4 bases y corta si
    calidad promedio es menor a 20.

-   `MINLEN:80` Descarta reads con menos de 80 bases después del
    trimming

El script correspondiente se puede encontrar dentro del GitHub, en la
ruta
[scripts/03_Trimmed.sh](https://github.com/leonardoglez6/Equipo2/blob/main/scripts/03_Trimmed.sh)

Los datos después del trimming, se almacenaron en el cluster, en la ruta
`/mnt/data/bioinfo-estadistica-2/RNAseq_2026/equipos/Equipo2/data/processed`

## 4. Análisis de calidad de las secuencias de los datos procesados

Seguido del trimming, se realizó el análisis de calidad de las 6
muestras con el programa **FastQC**, primero de forma individual y
después generando un MultiQC.

El script correspondiente se puede encontrar dentro del GitHub, en la
ruta
[scripts/04_Quality_trimmed.sh](https://github.com/leonardoglez6/Equipo2/blob/main/scripts/04_Quality_trimmed.sh)

Los resultados del análisis de calidad, se almacenaron en el cluster, en
la ruta
`/mnt/data/bioinfo-estadistica-2/RNAseq_2026/equipos/Equipo2/quality2`

Se obtuvo un contenido de GC aproximado de 50% formando una distribución
normal, característica de un resultado esperado de buena calidad, y una
cobertura que varía del rango de 17.9M - 23.2M.

![Contenido de GC de los datos
procesados](figures/fastqc_per_sequence_gc_content_plot_processed.png)

Las muestras tienen una calidad óptima dentro de las posiciones de las
lecturas, con valores dentro de la zona verde y un puntaje Phred
superior a 36.

![Calidad por base de los datos
procesados](figures/fastqc_per_base_sequence_quality_plot_processed.png)

![Calidad por secuencia de los datos
procesados](figures/fastqc_per_sequence_quality_scores_plot_processed.png)

El porcentaje de contenido de bases indeterminadas (N) en los reads
superó la prueba, así como la prueba de secuencias sobrerepresentadas y
no se encontraron muestras con contaminación de adaptadores los cuales
fueron removidos utilizando `trimmomatic` versión 0.33. A su vez,
realizar el trimming nos ayudó a quitar lecturas de mala calidad.

![Contenido de bases indeterminadas de los datos
procesados](figures/fastqc_per_base_n_content_plot_processed.png)

![Pruebas secuencias sobrerrepresentadas y contenido de adaptadores de
los datos
procesados](figures/fastqc_adapters_processed.png){width="1116"}

La presencia de secuencias duplicadas se mantuvo como una prueba
fallida, sin embargo, son resultados esperados al realizar RNA-seq.

![Secuencias duplicadas de los datos
procesados](figures/fastqc_sequence_duplication_levels_plot_processed.png)

Se encuentra un sesgo en las primeras 15 bases en la gráfica de
contenido por base, sin embargo, se reporta que es normal para RNA-seq.

![Contenido por
base](figures/fastqc_per_base_sequence_content_processed.png)

### Conclusión de la calidad de datos procesados

El procesamiento de los datos utilizando `trimmomatic` redujo la
cobertura de las secuencias, indicativo que removió lecturas de mala
calidad, y todos los reads pasaron la prueba de contenido de
adaptadores, por lo cuál indica que fueron removidos.

El tamaño de las reads iniciales (datos crudos) y las finales (datos
procesados) se mantuvo igual.

## 5. Generar index de STAR

Antes de realizar el alineamiento de las secuencias, es fundamental
realizar el indexado para acceder a los datos de forma más eficiente.
Para ello se obtuvo el genoma de referencia humano desde NCBI, versión
*GRCh38.p14*, en formato FASTA (secuencia) y GTF (anotaciones).

El genoma y anotaciones se almacenaron dentro del cluster, en la ruta
`/mnt/data/bioinfo-estadistica-2/RNAseq_2026/equipos/Equipo2/reference`

Posteriormente, se realizó el indexado con STAR, utilizando los
siguientes parámetros:

-   `--run ThreadN 30` Usa 30 threads en paralelo para mayor velocidad

-   `--runMode genomeGenerate` Genera el índice del genoma

-   `--genomeDir ./align/STAR/STAR_index` Indica ruta donde se guardarán
    los índices

-   `--genomeFastaFiles` Recibe el archivo FASTA del genoma de
    referencia

-   `--sjdbGTFfile` Recibe el archivo GTF del genoma de referencia

-   `--sjdbOverhang` Define el tamaño de la secuencia alrededor de los
    splice junction (punto de unión entre dos exones), el valor óptimo
    es `longitud read - 1`

El script correspondiente se puede encontrar dentro del GitHub, en la
ruta
[scripts/05_STAR_index.sh](https://github.com/leonardoglez6/Equipo2/blob/main/scripts/05_STAR_index.sh)

Los resultados del indexado, se almacenaron en el cluster, en la ruta
`/mnt/data/bioinfo-estadistica-2/RNAseq_2026/equipos/Equipo2/align/STAR/STAR_index`

## 6. Alineamiento de secuencias con STAR

Se realizó el alineamiento de las secuencias con STAR, debido a su alta
precisión y velocidad de mapeo. Para ello, se utilizaron las secuencias
de los datos procesados y el índice previamente generado. Además, se
realizó el conteo de las lecturas por genes. Se utilizaron los
siguientes parámetros:

-   `--runThreadN 12` Usa 12 threads en paralelo para mayor velocidad

-   `--genomeDir` Recibe la ruta del índice

-   `--readFilesIn` Recibe las secuencias de los datos procesados (ambas
    lecturas paired-end)

-   `--outSAMtype BAM SortedByCoordinate` Convierte archivo de salida
    SAM a BAM y ordenados por posición en el cromosoma

-   `--quantMode GeneCounts` Cuenta lecturas por genes

-   `--readFilesCommand zcat` Descomprime datos mientras los analiza

-   `--outFileNamePrefix` Define ruta donde se guardan resultados por
    muestra

El script correspondiente se puede encontrar dentro del GitHub, en la
ruta
[scripts/06_STAR_align.sh](https://github.com/leonardoglez6/Equipo2/blob/main/scripts/06_STAR_align.sh)

Los resultados del alineamiento, se almacenaron en el cluster, en la
ruta
`/mnt/data/bioinfo-estadistica-2/RNAseq_2026/equipos/Equipo2/align/STAR/STAR_output`

## 7. Importar datos de cuentas en R

Posterior al alineamiento con STAR, es necesario importar los datos
provenientes de este a R para el posterior análisis de Expresion
diferencial con DESEq2. En nuestro caso, realizamos el análisis con R
versión 4.4.1 dentro del cluster.

El script correspondiente al alineamiento se puede encontrar dentro del
GitHub, en la ruta
[scripts/07_Load_data_STAR.R](https://github.com/leonardoglez6/Equipo2/blob/main/scripts/07_Load_data_STAR.R)

Al terminar este script, contamos con una matriz de cuentas crudas de
los datos y cargamos los metadatos. Ambos se encuentran en la ruta de
github `results/counts/raw_counts.RData` como un dataset de R para
cargar.

Y, dentro del cluster, en la ruta
`/mnt/data/bioinfo-estadistica-2/RNAseq_2026/equipos/Equipo2/results/counts`

## 8. Análisis de expresión diferencial de genes (DEGs)

Cargamos nuestra matriz de cuentas crudas generada y la metadata para,
posteriormente, seleccionar los genes con más de 10 cuentas.

Al convertirlas al formato dds, observamos que teníamos 6 muestras y
17,266 genes hasta este punto. Para después realizar las comparaciones,
indicando cual es la referencia ("WT"). Guardamos el resultado de la
salida en el archivo `results/STEAP3_KD_vs_WT.RData` el cual contiene la
metadata y el archivo dds.

Posteriormente, se realizó la normalización de los datos para que poder
comparar los niveles de expresión entre muestras, evitando sesgos
técnicos y biológicos. Utilizamos la función rlog debido a que tenemos
pocas muestras (\<30).

Con el objeto resultante, se realizó un PCA con el objetivo de detectar
batch effect. De esta forma, se observó que las 3 muestras con la
condición Knockdown *STEAP3* se agrupan, mientras que las 2 de las WT se
agrupan y la muestra restante se aleja de los dos clusters.

![PCA antes de corrección por *batch
effect*](figures/PCA_rlog.png){width="1181"}

Al realizar la corrección por el batch effect con Limma, se obtiene la
misma gráfica y no se genera la columna `$batch` en el objeto generado
por DESeq2, por lo que se concluye que no existe batch effect y
probablemente existe una dispersión de los datos para las muestras WT
que no puede visualizarse debido a la poca cantidad de muestras (3).

![PCA después de corrección por *batch
effect*](figures/PCA_no_batch_rlog.png){width="1186"}

Finalmente, se obtuvo la información de las muestras con el único
contraste entre Wild-Type y Knockdown, que se guardó en el archivo
`results/DE_KD_vs_WT.csv`. En el resumen de este, podemos observar que
dentro de los 17,266 genes previamente filtrados hubieron:

-   744 con un LFC \> 0 (sobreexpresados)

-   825 con un LFC \< 0 (subexpresados)

-   0 outliers

-   4687 con pocas cuentas

El script correspondiente se puede encontrar dentro del GitHub, en la
ruta
[scripts/08_DEG_STAR_KD.R](https://github.com/leonardoglez6/Equipo2/blob/main/scripts/08_DEG_STAR_KD.R)

## 9. Visualización de los datos DEG

En primera instancia, se filtraron los genes que tuvieran un p-valor
ajustado menor a 0.05. Además, se clasificaron como sobreexpresados
cuando tuvieran LFC ≥ 2, y como subexpresados con LFC ≤ 2.

Algunas de las gráficas generadas a partir de los datos de los genes con
expresión diferenciada (DEG) se muestran a continuación. Para generarlas
se utilizaron los paquetes de `ggplot2` y `pheatmap`

El script correspondiente se puede encontrar dentro del GitHub, en la
ruta
[scripts/09_Visualization_data_STAR.R](https://github.com/leonardoglez6/Equipo2/blob/main/scripts/09_Visualization_data_STAR.R)

Las gráficas generadas en este paso y el siguiente se guardaron en el
cluster, en la ruta
`/mnt/data/bioinfo-estadistica-2/RNAseq_2026/equipos/Equipo2/results/figures`

### Volcano Plot

Para el Volcano Plot, observamos 3 genes subexpresados y 12 genes
sobreexpresados. El gen subexpresados con mayor p-value es *STEAP3* y el
gen sobreexpresado con mayor p-value es *SERPINB3*.

![Volcano Plot](results/figures/VolcanoPlot_KD_vs_WT.png)

### Heatmap

Por otro lado, realizamos un Heatmap, donde obtuvimos los 20 genes con
el p-valor más significativo, y creamos un filtro donde el Log Fold
Change máximo fuera 3, de esta manera evitamos sesgar los resultados por
los outliers.

![Heatmap](results/figures/Heatmap_KD_vs_WT.png)

## 10. Análisis funcional con términos GO

Se realizó el mismo filtrado que realizamos en la sección de
"Visualización de los datos" para realizar las gráficas..
Posteriormente, realizamos la determinación funcional de los genes
diferencialmente expresados a partir de los datos.

El script correspondiente se puede encontrar dentro del GitHub, en la
ruta
[scripts/10_GO_terms_STAR.R](https://github.com/leonardoglez6/Equipo2/blob/main/scripts/10_GO_terms_STAR.R)

### Manhattan plot

Primero, realizamos un Manhattan plot. Mediante este plot podemos
observar la cantidad de vías metabólicas observadas en cada base de
datos, representadas por un punto del color correspondiente.

Las bases de datos que escogimos fueron: Gene Ontology - Biological
Process (<GO:BP>), KEGG, Reactome (REAC), Transfac (TF), miRTarBase
(MIRNA), Human Protein Atlas (HPA), CORUM, Human Phenotype Ontology (HP)
y WikiPathways (WP).

![Manhattan
Plot](results/figures/ManhattanGO_KD_vs_WT.png){width="1377"}

### Gráficas de barras de genes sobreexpresados y subexpresados

Por otro lado, también podemos realizar una gráfica de barras con la
información de los genes subexpresados, ordenados por su p-valor y
categoría. En esta gráfica podemos observar las vías metabólicas
relacionadas a los genes subexpresados, y de igual forma de que base de
datos provienen cada una de ellas.

Algunas vías relacionadas a los genes que disminuyen su expresión son:

-   **Ferroptosis**: una forma de muerte celular programada dependiente
    de hierro.

-   **Metabolismo del cobre y hierro:** importación de iones de cobre y
    hierro, concentración epática elevada de hierro, transporte y
    abosrbción de hierro, metabolismo del hierro en la placenta,
    desórdenes del metabolismo del hierro, homesotasis del cobre,
    saturación de la transferina (proteína que transporta hierro)

-   **Crecimiento** celular y regulación de *TP53*

-   **Contracción muscular**: contracción del corazón, músculo liso de
    la vena, músculo liso arterial, músculo liso tónico, etc

-   **Ciclos de GTPasas:** *RHOJ*, *RHOO*, *RHOF*, *RHOD*

![Gráfica de barras de rutas metabólicas relacionadas a genes
subexpresados](results/figures/barplotDOWN_GO_KD_vs_WT.png)

De igual forma podemos observar las vías metabólicas relacionadas a los
genes sobreexpresados. Algunas vías son:

-   **Proteólisis**: regulación negativa de actividad de peptidasas,
    hidrolasas, actividad catalítica y funciones moleculares

-   **Respuesta inmune:** respuesta a virus, respuesta simbionte,
    respuesta a otros organismos, estímulos bióticos o estímulos
    externos, respuesta de defensa, regulación del sistema inmune,
    regulación de linfocitos, respuesta a estrés, señalización autócrina
    y parácrina

![Gráfica de barras de rutas metabólicas relacionadas a genes
sobreexpresados](results/figures/barplotUP_GO_KD_vs_WT.png)

# Discusión

En el artículo de origen se menciona que se obtuvieron 151 genes
sobreexpresados y 91 genes subexpresados, resultado el cuál difiere
bastante de lo obtenido por nosotros (12 sobreexpresados y 3
subexpresados). Esto posiblemente puede ser explicado por el hecho de
que los autores originales utilizaron diferentes programas a lo largo
del análisis bioinformático (alineamiento con HISAT2, mapeo de lecturas
con StringTie, mezcla de los transcriptomas con gffcompare, conteo de
lecturas con StringTie y ballgown), lo cual como posible resultado
permitió que los autores filtraran el análisis de expresión diferencial
de genes (DEGs) utilizando la métrica de False Discovery Rate (FDR) con
un treshold de \< 0.05, en lugar de aplicarlo para el p-value como se
realizó en este reporte, incluso al haberlo realizado con DESeq2 al
igual que los autores.

La siguiente tabla pertenece a una tabla suplementaria del artículo en
la cual enlista todos los DEGs, su Log2FoldChange, p-value y FDR.

```{r echo=FALSE, message=FALSE, warning=FALSE}
filas <- rbind(
  c("STEAP3", 0.18, -2.46, 0.00, 0.00),
  c("SERPINB3", 5.29, 2.40, 0.00, 0.00),
  c("IFI44L", 3.77, 1.92, 0.00, 0.00),
  c("IFI16", 3.53, 1.82, 0.00, 0.00),
  c("SERPINB4", 6.32, 2.66, 0.00, 0.00),
  c("RGS2", 2.80, 1.48, 0.00, 0.00),
  c("CXCL10", 9.87, 3.30, 0.00, 0.00),
  c("RSAD2", 5.79, 2.53, 0.00, 0.00),
  c("OLR1", 2.67, 1.41, 0.00, 0.00),
  c("CPA4", 0.44, -1.17, 0.00, 0.00),
  c("ESPN", 0.32, -1.64, 0.00, 0.00),
  c("HERC5", 2.41, 1.27, 0.00, 0.00),
  c("YWHAE", 0.47, -1.09, 0.00, 0.00),
  c("CFB", 2.17, 1.12, 0.00, 0.00),
  c("IFIT1", 2.19, 1.13, 0.00, 0.00),
  c("IFIH1", 2.34, 1.23, 0.00, 0.00),
  c("TPM4", 2.05, 1.04, 0.00, 0.00),
  c("TM4SF1", 2.22, 1.15, 0.00, 0.00),
  c("ADGRB2", 0.42, -1.24, 0.00, 0.00),
  c("IFIT2", 2.16, 1.11, 0.00, 0.00),
  c("ALDH3A1", 0.31, -1.69, 0.00, 0.00),
  c("OAZ1", 0.50, -1.00, 0.00, 0.00),
  c("GSN", 0.49, -1.02, 0.00, 0.00),
  c("UCP2", 0.49, -1.04, 0.00, 0.00),
  c("IL7R", 2.65, 1.41, 0.00, 0.00),
  c("GDA", 0.46, -1.11, 0.00, 0.00),
  c("CXCL11", 3.26, 1.71, 0.00, 0.00),
  c("SAMD9L", 3.60, 1.85, 0.00, 0.00),
  c("MMGT1", 0.42, -1.26, 0.00, 0.00),
  c("FN1", 2.48, 1.31, 0.00, 0.00),
  c("CTSL", 2.03, 1.02, 0.00, 0.00),
  c("SLC2A3", 2.31, 1.21, 0.00, 0.00),
  c("IFIT3", 2.24, 1.16, 0.00, 0.00),
  c("TENT5C", 2.01, 1.01, 0.00, 0.00),
  c("MESD", 2.40, 1.26, 0.00, 0.00),
  c("DDX58", 2.53, 1.34, 0.00, 0.00),
  c("ACSL4", 2.12, 1.08, 0.00, 0.00),
  c("IFI44", 2.42, 1.28, 0.00, 0.00),
  c("IFI6", 2.91, 1.54, 0.00, 0.00),
  c("DNAJC12", 2.29, 1.19, 0.00, 0.00),
  c("SAMD9", 2.85, 1.51, 0.00, 0.00),
  c("C4B", 2.82, 1.50, 0.00, 0.00),
  c("SAR1B", 0.41, -1.30, 0.00, 0.00),
  c("PLAC4", 2.78, 1.48, 0.00, 0.00),
  c("OAS2", 2.79, 1.48, 0.00, 0.00),
  c("SOD2", 2.22, 1.15, 0.00, 0.00),
  c("ERO1A", 2.14, 1.10, 0.00, 0.00),
  c("TDRD7", 2.27, 1.19, 0.00, 0.00),
  c("ENSG00000271741", 23.65, 4.56, 0.00, 0.00),
  c("SLC4A11", 0.41, -1.27, 0.00, 0.00),
  c("AGAP2-AS1", 4.99, 2.32, 0.00, 0.00),
  c("MUC13", 2.35, 1.23, 0.00, 0.00),
  c("FGG", 8.70, 3.12, 0.00, 0.00),
  c("ZW10", 2.11, 1.08, 0.00, 0.00),
  c("KRT16", 2.64, 1.40, 0.00, 0.00),
  c("ATOH8", 0.46, -1.13, 0.00, 0.00),
  c("GANAB", 2.34, 1.23, 0.00, 0.00),
  c("HCAR2", 3.17, 1.66, 0.00, 0.00),
  c("ERG", 2.10, 1.07, 0.00, 0.00),
  c("GLTPD2", 0.00, -13.24, 0.00, 0.00),
  c("IL6", 3.29, 1.72, 0.00, 0.00),
  c("TAX1BP3", 0.46, -1.12, 0.00, 0.00),
  c("IFIT5", 2.14, 1.10, 0.00, 0.00),
  c("HTR1D", 0.35, -1.52, 0.00, 0.00),
  c("PHC1", 0.47, -1.08, 0.00, 0.00),
  c("MAD2L1", 0.49, -1.04, 0.00, 0.00),
  c("PLSCR1", 2.08, 1.06, 0.00, 0.00),
  c("MCF2L", 2.50, 1.32, 0.00, 0.00),
  c("ADORA1", 0.33, -1.59, 0.00, 0.00),
  c("PEDS1-UBE2V1", 0.00, -14.13, 0.00, 0.00),
  c("CCN3", 2.12, 1.08, 0.00, 0.00),
  c("SLC2A14", 2.78, 1.48, 0.00, 0.00),
  c("TRIM22", 2.66, 1.41, 0.00, 0.00),
  c("TCIM", 2.18, 1.12, 0.00, 0.00),
  c("FAXDC2", 0.35, -1.52, 0.00, 0.00),
  c("INO80B-WBP1", 20005.57, 14.29, 0.00, 0.00),
  c("GPNMB", 2.02, 1.02, 0.00, 0.00),
  c("CHRND", 2.35, 1.23, 0.00, 0.00),
  c("ENSG00000271793", 0.00, -11.41, 0.00, 0.00),
  c("FGL1", 4.14, 2.05, 0.00, 0.00),
  c("NCF2", 2.03, 1.02, 0.00, 0.00),
  c("POLR2J2", 0.30, -1.75, 0.00, 0.00),
  c("STIMATE-MUSTN1", 0.40, -1.34, 0.00, 0.00),
  c("AATK", 0.38, -1.39, 0.00, 0.00),
  c("CDK20", 0.41, -1.30, 0.00, 0.00),
  c("OTUD1", 2.11, 1.08, 0.00, 0.00),
  c("C5AR1", 0.49, -1.04, 0.00, 0.00),
  c("ENSG00000286235", 0.00, -10.92, 0.00, 0.00),
  c("POC1B-GALNT4", 52.77, 5.72, 0.00, 0.00),
  c("SULF1", 2.64, 1.40, 0.00, 0.00),
  c("CFAP119", 0.43, -1.23, 0.00, 0.00),
  c("JSRP1", 0.35, -1.50, 0.00, 0.00),
  c("CCDC177", 2.57, 1.36, 0.00, 0.00),
  c("MAFA", 0.41, -1.29, 0.00, 0.00),
  c("ENSG00000244255", 0.00, -10.88, 0.00, 0.00),
  c("GAS6-AS1", 0.50, -1.00, 0.00, 0.00),
  c("ENSG00000284057", 0.00, -10.91, 0.00, 0.00),
  c("ENSG00000269693", 1761.81, 10.78, 0.00, 0.00),
  c("EDN2", 0.19, -2.39, 0.00, 0.00),
  c("ADAP1", 2.04, 1.03, 0.00, 0.00),
  c("IL1R1", 2.08, 1.06, 0.00, 0.00),
  c("IFNL1", 6.16, 2.62, 0.00, 0.00),
  c("KRT17", 2.12, 1.09, 0.00, 0.00),
  c("CEACAM1", 2.20, 1.13, 0.00, 0.00),
  c("ENSG00000286999", 0.00, -10.11, 0.00, 0.00),
  c("ENSG00000267062", 24097.16, 14.56, 0.00, 0.00),
  c("ENSG00000256427", 2.23, 1.15, 0.00, 0.00),
  c("HCAR3", 2.79, 1.48, 0.00, 0.00),
  c("IL32", 2.62, 1.39, 0.00, 0.00),
  c("ENSG00000251357", 6487.91, 12.66, 0.00, 0.00),
  c("ENSG00000286231", 2924.82, 11.51, 0.00, 0.00),
  c("IL17RE", 0.47, -1.10, 0.00, 0.00),
  c("C4A", 2.35, 1.23, 0.00, 0.00),
  c("CAPN5", 0.41, -1.28, 0.00, 0.00),
  c("CDK14", 2.00, 1.00, 0.00, 0.00),
  c("CYP1A1", 0.43, -1.20, 0.00, 0.00),
  c("GBP3", 2.10, 1.07, 0.00, 0.00),
  c("MMP24", 0.45, -1.16, 0.00, 0.00),
  c("HES6", 0.47, -1.08, 0.00, 0.00),
  c("IQCN", 0.48, -1.04, 0.00, 0.00),
  c("CACNA1G", 0.46, -1.11, 0.00, 0.00),
  c("UNC5CL", 2.53, 1.34, 0.00, 0.00),
  c("PLA2G3", 0.45, -1.14, 0.00, 0.00),
  c("GPC2", 0.47, -1.09, 0.00, 0.00),
  c("NCAM2", 2.19, 1.13, 0.00, 0.00),
  c("ENSG00000287188", 2.75, 1.46, 0.00, 0.00),
  c("FBXO32", 2.11, 1.08, 0.00, 0.00),
  c("GBP5", 6.28, 2.65, 0.00, 0.00),
  c("CFH", 2.16, 1.11, 0.00, 0.00),
  c("RBMS3", 2.10, 1.07, 0.00, 0.00),
  c("CP", 2.44, 1.29, 0.00, 0.00),
  c("IFI27", 4.50, 2.17, 0.00, 0.00),
  c("TGM2", 2.03, 1.02, 0.00, 0.00),
  c("ZNF845", 2.27, 1.18, 0.00, 0.00),
  c("PMS2P14", 7.36, 2.88, 0.00, 0.00),
  c("ZNF17", 2.30, 1.20, 0.00, 0.00),
  c("H19", 0.40, -1.31, 0.00, 0.00),
  c("TLR3", 2.42, 1.28, 0.00, 0.00),
  c("CRLF2", 2.80, 1.49, 0.00, 0.00),
  c("ZCWPW1", 2.51, 1.33, 0.00, 0.00),
  c("ENSG00000283549", 0.00, -11.98, 0.00, 0.00),
  c("CYSRT1", 0.35, -1.52, 0.00, 0.00),
  c("LINC01285", 437.54, 8.77, 0.00, 0.00),
  c("SOCS3-DT", 2.33, 1.22, 0.00, 0.00),
  c("MX2", 5.46, 2.45, 0.00, 0.00),
  c("CAPN3", 2.06, 1.05, 0.00, 0.00),
  c("MYOM1", 2.13, 1.09, 0.00, 0.00),
  c("ENSG00000167774", 0.13, -2.89, 0.00, 0.00),
  c("CFAP70", 2.18, 1.13, 0.00, 0.00),
  c("HPCAL4", 0.00, -8.99, 0.00, 0.00),
  c("ITPKA", 0.49, -1.02, 0.00, 0.00),
  c("PARD6A", 0.42, -1.25, 0.00, 0.00),
  c("ABCA1", 2.33, 1.22, 0.00, 0.00),
  c("LPAR5", 0.44, -1.19, 0.00, 0.00),
  c("GLI1", 0.43, -1.22, 0.00, 0.00),
  c("ADIRF", 0.36, -1.49, 0.00, 0.00),
  c("CEMIP", 2.16, 1.11, 0.00, 0.00),
  c("ALPG", 0.48, -1.07, 0.00, 0.00),
  c("DOCK8", 0.50, -1.00, 0.00, 0.00),
  c("WBP4", 2.03, 1.02, 0.00, 0.00),
  c("RXFP1", 4.67, 2.22, 0.00, 0.00),
  c("HOXC13-AS", 2.07, 1.05, 0.00, 0.00),
  c("AIM2", 332.86, 8.38, 0.00, 0.00),
  c("ACTL8", 2.40, 1.26, 0.00, 0.00),
  c("ENSG00000261888", 3.87, 1.95, 0.00, 0.01),
  c("THSD7B", 308.47, 8.27, 0.00, 0.01),
  c("SELPLG", 2.01, 1.01, 0.00, 0.01),
  c("SEPTIN5", 0.03, -5.19, 0.00, 0.01),
  c("HTR3A", 4.07, 2.03, 0.00, 0.01),
  c("ENSG00000289517", 47.40, 5.57, 0.00, 0.01),
  c("ENSG00000241489", 3.44, 1.78, 0.00, 0.01),
  c("EGR2", 0.28, -1.82, 0.00, 0.01),
  c("ATP1A3", 0.18, -2.45, 0.00, 0.01),
  c("ENSG00000269826", 2042.00, 11.00, 0.00, 0.01),
  c("UCA1", 3.43, 1.78, 0.00, 0.01),
  c("PLAC8L1", 3.35, 1.74, 0.00, 0.01),
  c("ALOX15B", 0.28, -1.83, 0.00, 0.01),
  c("RNLS", 4.07, 2.02, 0.00, 0.01),
  c("PRRT4", 0.00, -9.05, 0.00, 0.01),
  c("C2CD4D-AS1", 3.31, 1.73, 0.00, 0.01),
  c("SLC26A7", 282.86, 8.14, 0.00, 0.01),
  c("IL22RA2", 697.41, 9.45, 0.00, 0.01),
  c("ENSG00000244332", 0.22, -2.15, 0.00, 0.01),
  c("SGIP1", 74.20, 6.21, 0.00, 0.01),
  c("PRICKLE2", 2.50, 1.32, 0.00, 0.02),
  c("PSG4", 2.73, 1.45, 0.00, 0.02),
  c("GCOM1", 0.49, -1.03, 0.00, 0.02),
  c("NANOS1", 0.43, -1.22, 0.00, 0.02),
  c("ABCA3", 0.49, -1.02, 0.00, 0.02),
  c("ENSG00000261606", 0.43, -1.20, 0.00, 0.02),
  c("BCRP7", 0.00, -11.57, 0.00, 0.02),
  c("PRXL2B", 0.47, -1.09, 0.00, 0.02),
  c("DISC1", 2.49, 1.32, 0.00, 0.02),
  c("TNFSF14", 3.19, 1.67, 0.00, 0.02),
  c("ENSG00000282418", 0.00, -11.46, 0.00, 0.02),
  c("ACTBL2", 4.13, 2.05, 0.00, 0.02),
  c("STAC3", 2.26, 1.18, 0.00, 0.02),
  c("HELZ2", 2.46, 1.30, 0.00, 0.02),
  c("SBK2", 0.48, -1.06, 0.00, 0.02),
  c("SUCNR1", 18.02, 4.17, 0.00, 0.02),
  c("ENSG00000272476", 2.22, 1.15, 0.00, 0.02),
  c("ARC", 0.25, -2.01, 0.00, 0.02),
  c("KLHL30", 0.46, -1.13, 0.00, 0.03),
  c("ENSG00000261915", 41.37, 5.37, 0.00, 0.03),
  c("ZNF354C", 0.00, -8.01, 0.00, 0.03),
  c("ENSG00000289260", 3.36, 1.75, 0.00, 0.03),
  c("ENSG00000260927", 3.91, 1.97, 0.00, 0.03),
  c("NR4A1AS", 0.45, -1.14, 0.00, 0.03),
  c("ENSG00000284719", 602.08, 9.23, 0.00, 0.03),
  c("SCN4A", 0.49, -1.02, 0.00, 0.03),
  c("HSH2D", 2.39, 1.26, 0.00, 0.03),
  c("EDA2R", 0.45, -1.16, 0.00, 0.03),
  c("MFSD2B", 0.02, -5.41, 0.00, 0.03),
  c("KCNH3", 0.30, -1.75, 0.00, 0.03),
  c("ALDH2", 2.14, 1.10, 0.00, 0.03),
  c("WBP1LP2", 2.82, 1.50, 0.00, 0.03),
  c("TOGARAM2", 0.11, -3.21, 0.00, 0.03),
  c("ZNF117", 2.04, 1.03, 0.00, 0.03),
  c("CSAG2", 2.73, 1.45, 0.00, 0.03),
  c("ENSG00000225339", 0.02, -5.47, 0.00, 0.03),
  c("ACSBG1", 0.37, -1.42, 0.00, 0.03),
  c("C10orf67", 13.68, 3.77, 0.00, 0.03),
  c("TNFAIP6", 4.01, 2.00, 0.00, 0.03),
  c("ST18", 100.95, 6.66, 0.00, 0.04),
  c("CPLX3", 0.40, -1.31, 0.00, 0.04),
  c("SALRNA1", 671.33, 9.39, 0.00, 0.04),
  c("RAET1L", 2.54, 1.34, 0.00, 0.04),
  c("ENSG00000259621", 5425.58, 12.41, 0.00, 0.04),
  c("C2CD4C", 0.35, -1.51, 0.00, 0.04),
  c("LINC01050", 730.37, 9.51, 0.00, 0.04),
  c("FAM83A-AS1", 0.35, -1.51, 0.00, 0.04),
  c("PATL2", 2.74, 1.46, 0.00, 0.04),
  c("PRR15L", 0.00, -9.80, 0.00, 0.04),
  c("PLCB4", 63.23, 5.98, 0.00, 0.04),
  c("CITED1", 0.05, -4.23, 0.00, 0.04),
  c("FERMT3", 3.06, 1.61, 0.00, 0.05),
  c("SMCO3", 0.00, -9.28, 0.00, 0.05),
  c("LINC01939", 864.58, 9.76, 0.00, 0.05),
  c("PMS2P10", 0.38, -1.40, 0.00, 0.05),
  c("ENSG00000241269", 2322.20, 11.18, 0.00, 0.05),
  c("LINC02945", 2.58, 1.37, 0.00, 0.05),
  c("KRT17P4", 0.00, -8.80, 0.00, 0.05)
)

tabla_S4 <- data.frame(
  Gene    = filas[, 1],
  FC      = as.numeric(filas[, 2]),
  Log2FC  = as.numeric(filas[, 3]),
  P_value = as.numeric(filas[, 4]),
  FDR     = as.numeric(filas[, 5]),
  stringsAsFactors = FALSE
)

datatable(
  tabla_S4,
  colnames    = c("Gene", "FC", "Log2 FC", "P value", "FDR"),
  rownames    = FALSE,
  caption = "Tabla 3. Genes diferencialmente expresados (DEGs) obtenidos en el artículo",
  filter      = "top",
  options     = list(
    pageLength  = 10)
)
```

El volcano plot que se realizó en el artículo presenta todos sus DEGs,
resaltando que los mismos genes con mayor p-value de cada grupo
coinciden con los reportados en nuestro trabajo.

![Volcano Plot del artículo "Silencing of *STEAP3* suppresses cervical
cancer cell proliferation and migration via JAK/STAT3 signaling
pathway."](figures/Volcanoplot-articulo-original.jpeg)

Además el artículo presenta un dotplot en el cual se visualizan las 20
rutas más enriquecidas en KEGG, de la cual destacan la Vía de
señalización JAK-STAT. Hacen énfasis debido a la participación del gen
*STEAP3* ya que debido a diversos estudios en los cuales se realizaron
mediciones de la expresión del gen y de marcas de metilación en la
secuencia del gen en tejido de cáncer cervical, muestran significancia
en comparación a una condición celular normal; además que encontraron
una asociación con la capacidad de migración de las células cancerígenas
en función de la expresión de este gen.

Sin embargo, al contrario de ellos, debido a nuestro reducido número de
DEGs, dicha vía de señalización no se encuentra enriquecida en nuestro
análisis.

![Dotplot del artículo "Silencing of *STEAP3* suppresses cervical cancer
cell proliferation and migration via JAK/STAT3 signaling
pathway."](figures/Dotplot-articulo-original.jpeg){width="427"}


---

Por otro lado, la importancia de la expresión de *STEAP3* no solo ha
sido reportada para el cáncer de cuello uterino. Como se menciona en
Han, Y., *et al*., cuando *STEAP3* se encuentra sobreexpresado en
tejidos de cáncer de ovarios, se reporta una menor supervivencia.

Por lo tanto, mediante experimentos *in vitro* se demostró que al
silenciar *STEAP3* disminuye la proliferación, migración y capacidad
invasiva de células tumorales, esto al desencadenar procesos de
ferroptosis en las células cancerosas.

Un aspecto relevante es que existe una correlación negativa entre
*STEAP3* y p53. Donde, al silenciar *STEAP3*, los niveles de p53
aumentan, inhibiendo la expresión de *SLC7A11*, induciendo la
ferroptosis.

Con ello, podemos concluir que el silenciamiento de *STEAP3* puede
inhibir el crecimiento de diversos tumores y que puede hacerlo mediante
vías distintas a JAK-STAT.

---

Referente a los otros genes que tenían una expresión diferenciada,
pudimos complementar nuestro análisis al informarnos de otras funciones
de algunos genes y la relación con el cáncer cervical reportada en la
literatura.

Primeramente, los genes *SERPINB3* y *SERPINB4* son los genes
sobreexpresados que presentan un mayor p-value y tienen homología
divirgiendo principalmente en loop del sitio reactivo (RSL) en la región
de carboxyl terminal. Particularmente, *SERPINB3* es un inhibidor de
proteasa de cisteína intracelular que se ha reportado sobreexpresado en
cáncer.

En el estudio realizado por Wang, S., *et al.* se mostró que las células
de cáncer cervical son más sensibles a tratamiento, esto debido a que
este gen tiene funciones de protección ante daño lisosomal y muerte
organismal al inhibir la actividad de proteasa de cisteína inducida por
estreses como daño al DNA y especies reactivas de oxígeno.

---

Por otro lado, podemos analizar la relación de la sobreexpresión de
*CXCL10* con la supresión de la proliferación celular.

De acuerdo con Li, X., *et al.* donde se creo un adenovirus oncolítico
recombinante modificado genéticamente para expresar la quimiocina
*CXCL10*, la expresión continua de *CXCL10* en el interior del tumor,
atrae a las células T, que expresan el receptor *CXCR3* (ubicado en la
superficie de los linfocitos T citotóxicos CD8+ y CD4+).

Por lo tanto, la terapia con *CXCL10* suprime el crecimiento del tumor,
incrementando la infiltración de células T en el microambiente tumoral.

Podemos concluir que la sobreexpresión de este gen es coherente como una
respuesta de la célula para suprimir la proliferación celular.

---

Algunas limitaciones de nuestro análisis fue que se utilizaron
diferentes programas en el desarrollo de nuestro análisis, comparado con
el artículo de referencia. Esto redujo la cantidad de DEGs, por lo que
no se tuvieron resultados iguales. Sin embargo, logramos identificar
algunas similitudes en las rutas enriquecidas como ferroptosis. Creemos
que se podría mejorar el análisis al incorporar elementos de su flujo de
trabajo, pero manteniendo los criterios establecidos para un análisis de
calidad. De igual forma, podríamos mejorar nuestro trabajo al emplear
flujos de trabajo de Nextflow.

Otra limitación sería la poca cantidad de muestras obtenidas en el
experimento. Tener más muestras biológicas nos podría ayudar a que se
delimitaran más las diferencias entre muestras de un mismo grupo, como
se vio en el grupo control al analizar el PCA del batch effect. Esto nos
ayudaría a que nuestro análisis fuera más robusto.

# Referencias

-   Zhao, Z., Yu, P., Wang, Y., Li, H., Qiao, H., Sun, C., Zhu, L., &
    Yang, P. (2024). Silencing of *STEAP3* suppresses cervical cancer
    cell proliferation and migration via JAK/STAT3 signaling
    pathway. *Cancer & metabolism*, *12*(1), 40.
    <https://doi.org/10.1186/s40170-024-00370-2>

-   Terán, M., Mónaco, M., Haro, C., Ledesma, M. *et al*. Ferroptosis,
    un mecanismo de muerte celular presente en β-talasemia menor.
    (2024). *Revista Bioquímica Y Patología Clínica*, *89*(1),
    19-26. <https://doi.org/10.62073/k7g1yk82>

-   Han, Y., Fu, L., Kong, Y., Jiang, C., Huang, L., & Zhang, H. (2024).
    STEAP3 Affects Ovarian Cancer Progression by Regulating Ferroptosis
    through the p53/SLC7A11 Pathway. *Mediators of inflammation*,
    *2024*, 4048527. <https://doi.org/10.1155/2024/4048527>

-   Wang, S., Luke, C.J., Pak, S.C. *et al.* SERPINB3 (SCCA1) inhibits
    cathepsin L and lysoptosis, protecting cervical cancer cells from
    chemoradiation. *Commun Biol* **5**, 46 (2022).
    <https://doi.org/10.1038/s42003-021-02893-6>

-   Li, X., Lu, M., Yuan, M., Ye, J., Zhang, W., Xu, L., Wu, X., Hui,
    B., Yang, Y., Wei, B., Guo, C., Wei, M., Dong, J., Wu, X., & Gu, Y.
    (2022). CXCL10-armed oncolytic adenovirus promotes
    tumor-infiltrating T-cell chemotaxis to enhance anti-PD-1 therapy.
    *Oncoimmunology*, *11*(1), 2118210.
    <https://doi.org/10.1080/2162402X.2022.2118210>
